<template>
  <div class="app-container">
    ...
    <el-table :data="nodeList" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="header-center" label="replicas">
        <template slot-scope="scope">
          {{ scope.row.replicas }}
        </template>
      </el-table-column>
...
import { getDeployments, createDeployment, deleteDeployment, getNamespaces } from '@/api/kube'
...
export default {
  ...
  methods: {
    async getServiceList() {
      const res = await getDeployments(this.listQuery)
    ...
    handleDelete({ $index, row }) {
      this.$confirm('Confirm to remove the role?', 'Warning', {
        ...
        .then(async() => {
          await deleteDeployment({ name: row.name, namespace: row.namespace })
    ...
    async confirmRole() {
      ...
      if (isEdit) {
        ...
        await createDeployment(this.role)
