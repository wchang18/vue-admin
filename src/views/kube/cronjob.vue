<template>
  <div class="app-container">
    ...
    <el-table :data="nodeList" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="center" label="Schedule" width="220">
        <template slot-scope="scope">
...
import { getCronJobs, createCronJob, deleteCronJob, getNamespaces } from '@/api/kube'
...
export default {
  ...
  methods: {
    async getServiceList() {
      const res = await getCronJobs(this.listQuery)
    ...
    handleDelete({ $index, row }) {
      this.$confirm('Confirm to remove the role?', 'Warning', {
        ...
        .then(async() => {
          await deleteCronJob({ name: row.name, namespace: row.namespace })
